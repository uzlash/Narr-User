  ...
  <v-app class="custom__style">
    ...
    >
      ...
      <template v-slot:after-inner>
        <span title="after-inner" class="">Select image file</span>
      </template >
      <template v-slot:after-outer>
        <div title="after-outer">
          <p>Please select an image and click the upload button</p>
          <div class="">
            <p class="">Drop the file!</p>
          </div>
          <v-btn type="button" class="" :class="{'disabled': uploaded || !profilePic}" @click="upload()">Upload</v-btn>
          <v-btn type="button" class="btn" :class="[uploaded ? 'btn-danger' : 'btn-secondary']" v-if="profilePic" @click="removePic()">Remove</v-btn>
