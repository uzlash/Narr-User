<template>
  <v-app class="grey lighten-4">
        ...
        <v-col cols="12">
          ...
          >
            ...
            <template v-slot:[`item.actions`]="{ item }">
              <v-icon color="blue" class="mr-2" @click="viewCrowdFund(item.id)">
                mdi-eye
              </v-icon>
              <v-icon color="#00a368" class="mr-2" @click="editCrowdFund(item)">
                mdi-pencil
              </v-icon>
              <v-icon color="red" @click="deleteCrowdFund(item)">
                mdi-delete
              </v-icon>
            </template>
          </v-data-table>
    ...
    <v-dialog v-model="dialogAdd" max-width="800px">
      ...
      <v-card>
        <v-card-title class="font-weight-light"
          >Add New Crowd Funding</v-card-title
        >
        <v-card-text>
          <v-container>
            <v-row>
